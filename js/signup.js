const api = new API();
var signup_url =
	"https://store-manager-challenge-3.herokuapp.com/api/v2/auth/signup";
...
signupButton.addEventListener("click", e => {
	if (
		...
		msgBody.innerHTML =
			"both phone Nummber and password should atleast be 6 charcters more";
		...
		api.post(signup_url, user_data, token)
			.then(data => {
				if (data["message"] === "sorry username is already taken") {
					...
					data["message"] ===
					"sorry username shouldnt have a special character including ($#@%)"
				) {
			...
			.catch(error => {
				msgBody.innerHTML =
					"We are experiencing some difficulties now, please try again later";
				console.log(error);
			});
	}
});
