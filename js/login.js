const api = new API();
var login_url =
	"https://store-manager-challenge-3.herokuapp.com/api/v2/auth/login";
	...
	api.login(login_url, user_data)
		.then(data => {
			...
			if (user.value === "" || passwrd.value === "") {
				...
				data["message"] ===
				"sorry username and password dont match, please login with valid credentials"
			) {
				msgBody.innerHTML = `${data["message"]}`;
			} else if (
				data["message"] ===
				`Sorry user ${username} doesnot exist, login with valid credentials`
			) {
				msgBody.innerHTML =
					"You tried to login with a username that doesnot exist, login with valid credentials";
